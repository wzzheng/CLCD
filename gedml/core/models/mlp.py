...
class BatchNormLayer(WithRecorder):
    def __init__(self, in_dim, out_dim, is_bn=True, is_relu=True, is_bias=True, **kwargs):
        super(BatchNormLayer, self).__init__(**kwargs)
        layers_list = []
        # add linear 
        layers_list.append(nn.Linear(in_dim, out_dim, bias=is_bias))
        # add batch norm
        if is_bn:
...
class BatchNormMLP(WithRecorder):
    ...
    """
    def __init__(self, layer_size_list, relu_list, bn_list, first_bn=False, bias_list=[True], **kwargs):
        ...
        if len(bias_list) == 1:
            bias_list = bias_list * self.layers_num
        self.bias_list = bias_list
        assert len(self.bias_list) == self.layers_num
