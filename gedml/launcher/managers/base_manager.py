...
class BaseManager:
    ...
    def __init__(
        ...
        self.syncBN = syncBN and is_distributed
    ...
    def initiate_collectors(self):
        ...
        if self.syncBN:
            self.convert_sync_bn("collectors")
        if is_to_device:
    ...
    def convert_sync_bn(self, module_name: str):
        members = getattr(self, module_name)
        for k, v in members.items():
            members[k] = torch.nn.SyncBatchNorm.convert_sync_batchnorm(members[k])
    ...
    def run(self, phase="train", start_epoch=0, total_epochs=61, is_test=True, is_save=True, interval=1, warm_up=2, warm_up_list=None):
        ...
        self.maybe_resume()
        if start_epoch != 0:
            logging.info("resuming schedulers lr...")
            for v in self.schedulers.values():
                v.step(epoch=start_epoch)
