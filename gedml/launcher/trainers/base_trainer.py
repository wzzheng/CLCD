...
class BaseTrainer:
    ...
    def initiate_property(self):
        ...
        # recordable list
        self.recordable_object_list = ["models", "losses"]
        # trainable list
    ...
    def pipeline(self):
        ...
        if dist.is_initialized():
            ...
            rank = 0
        self.pbar = tqdm(range(self.iterations_per_epoch), disable=(rank!=0))
    ...
    def initiate_dataloader(self):
        ...
        if hasattr(self.datasets["train"], "get_grouped_idx"):
            logging.info("resetting train dataset...")
            self.datasets["train"].get_grouped_idx()
            if dist.is_initialized():
                grouped_idx = self.datasets["train"].grouped_idx.cuda(self.device)
                dist.broadcast(grouped_idx, 0)
                self.datasets["train"].grouped_idx = grouped_idx.cpu()
    ...
    def use_global_collector(self):
        for collector in self.collectors.values():
            if isinstance(collector, DistributedDataParallel):
    ...
    def _backward_and_update_preprocess(self):
        ...
        if self.is_distributed:
            ...
            # )
            dist.all_reduce(
                tensor=self.loss_handler.loss_values["total"],
                op=dist.ReduceOp.SUM
            )
